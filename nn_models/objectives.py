...
def OneClassNN_Loss(y_pred, y_true, v, rho, hidden_layer, decode_layer):
    """ OneClassNN_Loss.
    ...
    # Compute L2 norm of the Weights of the hidden- Output  layer
    term2 = tflearn.losses.L2(V)

    term3 = (1 / v) * 1 * tf.reduce_mean(tf.maximum(0.0, rho - y_pred) - rho)
...
def rPCA_autoencoderLoss(y_pred, y_true, mue, hidden_layer, decode_layer):
    """ Mean Square Loss.
    ...
    print("Shape of hidden layer weights....", e)
    print("Shape of decode layer weights....", d)

    term1 = tf.reduce_mean(tf.square(y_pred - y_true))
    # term2 = 0
    print("shape of term1", term1.get_shape())
    term2 = (t1) + t2
    term3 = 0
    cost = term1 + term2 + term3
...
#     """
#     cost = 0
#     with tf.name_scope("rPCA_autoencoderLoss"):
#         term1 = (y_pred - y_true)**2
#         term2 = mue/2
#         term3 = 0
#         cost = term1 + term2 + term3
#         return tf.reduce_mean(tf.square(y_pred - y_true) + (mue/2) )

def rPCA_autoencoderLoss_FobsquareLoss(y_pred, y_true, mue, hidden_layer, decode_layer):
    """ Mean Square Loss.
